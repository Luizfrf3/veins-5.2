...
ccas.fit(x0, x0)
X_c, Y_c = ccas.transform(x0, x0)
print(np.mean([np.corrcoef(X_c[:, i], Y_c[:, i])[0, 1] for i in range(10)]))
ccas.fit(x0, x1)
X_c, Y_c = ccas.transform(x0, x1)
print(np.mean([np.corrcoef(X_c[:, i], Y_c[:, i])[0, 1] for i in range(10)]))
ccas.fit(x0, x2)
X_c, Y_c = ccas.transform(x0, x2)
print(np.mean([np.corrcoef(X_c[:, i], Y_c[:, i])[0, 1] for i in range(10)]))
ccas.fit(x0, x3)
X_c, Y_c = ccas.transform(x0, x3)
print(np.mean([np.corrcoef(X_c[:, i], Y_c[:, i])[0, 1] for i in range(10)]))
ccas.fit(x0, x4)
X_c, Y_c = ccas.transform(x0, x4)
print(np.mean([np.corrcoef(X_c[:, i], Y_c[:, i])[0, 1] for i in range(10)]))
...
ccas = CCA(n_components=10, regs=0.01)
print(np.mean(ccas.stats(ccas.fit_transform([x0, x1]))["r"]))
print(np.mean(ccas.stats(ccas.fit_transform([x0, x2]))["r"]))
print(np.mean(ccas.stats(ccas.fit_transform([x0, x3]))["r"]))
print(np.mean(ccas.stats(ccas.fit_transform([x0, x4]))["r"]))
